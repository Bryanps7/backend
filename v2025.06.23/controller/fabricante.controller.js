...
const cadastrar = async (req, res) => {
    ...
    try {
        const post = await Fabricante.create(dados)
...
const listar = async (req, res) => {
    try {
        const listen = await Fabricante.findAll()
        if (!listen) {
...
const apagar = async (req, res) => {
    ...
    try {
        const search = await Fabricante.findByPk(id)
        if (!search) {
...
const atualizar = async (req, res) => {
    ...
    try {
        const search = await Fabricante.findByPk(id)
        if (!search) {
            await Fabricante.update(dados, { where: {codFabricante: id} })
